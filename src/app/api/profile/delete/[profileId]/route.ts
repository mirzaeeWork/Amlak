...
export async function DELETE(req: NextRequest, { params }: profileIDType) {
    try {
        await connectDB()
        ...
        const profile=await ProfileModel.findOneAndDelete({_id,userId:user._id})
        ...
        return NextResponse.json(getProfileSuccessResponse(
            [profile],
            ProfileResponseMessages.DELETED
          ));
